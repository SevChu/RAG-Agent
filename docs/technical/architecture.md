...
- 自动执行生成代码；
- 自动下载未审批的数据集或模型；
- 真实训练、模拟 Adapter 部署及自动质量晋级；AgentProfile 和模拟微调管理已实现。
...
## v1.4.0-beta.1：科研模拟训练子系统

训练子系统位于 `app/training`，通过三个独立 REST 前缀与前端训练页面交互；共用 SQLAlchemy
模型及 07～09 迁移，worker 在科研模式的 FastAPI lifespan 中按配置启动/关闭。单机持久租约协调
领取和完成，普通生成路径不调用 Trainer。资料索引队列与训练任务是两套生命周期，不能混为一谈。

源数据登记、任务快照和产物 manifest 各有哈希。SQLite 保存训练元数据，独立训练目录保存 JSONL
和模拟清单；文件/数据库一致性有明确失败与残留边界。完整架构和恢复见[模拟微调平台](training-platform.md)。
前端采用来源请求序号、复核版本、幂等键和终态停止轮询；产品页隐藏入口且后端继续验证能力。
