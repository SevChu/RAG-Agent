...
## v1.4.0-beta.1：资料管线与训练数据的边界

本版本未替换 Dense、Reranker、证据门控或 Qdrant 索引格式。训练 JSONL 经显式上传单独登记，
不从资料空间或会话自动收集；source_course_ids 用于资格与空间范围核验，不代表拷贝整个知识库。
JSONL/manifest 不写入向量库，模拟成功不触发重索引或在线重排切换。普通上传删除语义保持，涉及
训练来源的资格由训练服务及迁移触发器联动；历史元数据保留。备份新增训练目录，见[训练平台](training-platform.md)。
