...
└─ baseline_profiles.py          # 严格配置加载与校验
...
`baseline-profiles.json` 的 release version 为 `1.0.0`，文件 SHA-256 为
`d4874b327c5df503d010792848cd7ad5b6f30a761b41fd31c3cfb1e5e7cefe24`。三个 profile 为：
...
修改参数或算法时必须创建新 profile 版本或明确标记实验候选；不能静默覆盖 1.0.0 的参考数值。
...
- 幻觉下一候选：加入句子级 entailment/contradiction 特征，优先改善 AUPRC 与 span F1。
