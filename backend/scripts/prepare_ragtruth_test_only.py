...
DEFAULT_OUTPUT = DATASET_ROOT / "derived" / "test-only"
TRANSFORM_ID = "ragtruth-stream-filter-test-v1"
...
def _copy_test_responses(source: Path, target: Path) -> set[str]:
    ...
    with (
        ...
        for line_number, line in enumerate(input_handle, start=1):
            ...
            if response_id in response_ids:
                raise ValueError("test response ids must be unique")
            response_ids.add(response_id)
            test_source_ids.add(source_id)
            output_handle.write(line)
    if not response_ids:
        raise ValueError("RAGTruth test selection is empty")
    return test_source_ids


def _copy_test_sources(source: Path, target: Path, test_source_ids: set[str]) -> None:
    ...
    with (
        ...
        for line_number, line in enumerate(input_handle, start=1):
            if not line.strip():
                continue
            source_id = _extract_single_field(line, _SOURCE_ID_RE, "source_id", source, line_number)
            if source_id not in test_source_ids:
                continue
            ...
            if source_id in copied_source_ids:
                raise ValueError("selected test source ids must be unique")
            copied_source_ids.add(source_id)
            output_handle.write(line)

    missing = test_source_ids.difference(copied_source_ids)
    if missing:
        raise ValueError(f"selected responses reference {len(missing)} missing test sources")
...
def derive_test_only(
    raw_root: Path,
    output_root: Path,
    parent_manifest: RagTruthManifest,
    transform_path: Path,
) -> dict[str, Any]:
    """Build the approved frozen test view without decoding excluded train responses."""
    ...
    staging = Path(tempfile.mkdtemp(prefix=".test-only-", dir=output_root.parent))
    try:
        test_source_ids = _copy_test_responses(
        ...
        observed = RagTruthAdapter(staging).validate(("test",))["test"]
        expected = dict(parent_manifest.split_counts["test"])
        if observed != expected:
            raise ValueError(
                f"derived test counts differ from frozen parent manifest: {observed} != {expected}"
            )

        relative_files = ("response.jsonl", "source_info.jsonl")
        payload: dict[str, Any] = {
            "schema_version": "1.0",
            "dataset_id": "ragtruth-test-only",
            "lifecycle": "frozen",
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Create the approved frozen RAGTruth test-only derived snapshot"
    )
    ...
    payload = derive_test_only(
        dataset_root / "raw",
        args.output,
        parent_manifest,
        Path(__file__),
    )
    counts = payload["split_counts"]["test"]
    print(f"created={args.output.resolve()}")
    print(f"test_counts={_counts_text(counts)}")
