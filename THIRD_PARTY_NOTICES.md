...
产品/科研源码包分离，不新增或升级第三方包版本。PyArrow 与研究所需的 scikit-learn 直接
约束移至 research 组；scikit-learn 仍由产品检索依赖间接使用。产品默认不安装 PyArrow。
科研包只包含研究源码与已公开聚合证据，不含原始 Benchmark、逐样本预测、权重、私有数据
或预装依赖。两版均不得把本地研究成绩解释为客户部署保证。实际二进制、前端 bundle 或
离线依赖制品仍须另行生成 SBOM。
