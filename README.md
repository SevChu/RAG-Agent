...
> 的 LLM 回答与引用功能已实现；风格区分、证据推理和引用兼容性修复已通过用户针对性复测。
...
在本地 `.env` 填写 API Key 后，用户已完成首轮风格测试：详细模式基础质量较好，但简洁与
均衡区分不明显，且详细回答缺少衍生推理。当前已将三种风格改为明确的篇幅与结构契约，
详细模式新增“证据前提 → 推理过程 → 可得结论/应用”的证据锚定推导。正文引用现在作为
最终来源依据，并兼容单条及逗号、中文逗号、顿号组合引用，避免被模型清单排序差异误拦截。
用户复测建议验收第一个问题后确认回答明显改善。当前计划日 2 工程实现已收口；完整五问
结果尚未逐项记录，后续可继续作为质量评测样本。
Reranker、查询改写、SSE 和长期对话仍未开始，Reranker 模型也未下载。
