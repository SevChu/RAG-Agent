...
> 项目状态：第 1 周和第 2 周均已完成并通过验收；第 3 周计划日 1～3 已完成针对性验收。
...
只从配置读取，不写死在业务逻辑中。当前代码已能返回默认模型和白名单，但真实界面切换尚未
贯通：问答仍固定使用 `LLM_MODEL`。该能力安排在第 3 周计划日 4 实施；完成后，前端只能选择
后端返回的可用模型，后端必须再次执行白名单校验，并在回答中返回实际使用的模型。API Key
始终只保存在后端环境变量中。
...
- 实现基础检索、重排和引用；
- 计划日 4 优先收口真实生成模型切换，并建立 `Conversation`、`Message`、课程会话保存恢复和
  `conversation_id` 数据契约；
- 计划日 5 完成上下文感知查询改写、课程多轮 RAG、SSE 流式输出、快速对话和两类历史隔离；
...
针对性人工验收；该记录不扩写为八道建议题全部逐项通过。当前两个 DeepSeek 模型虽然已经
出现在配置白名单和设置页中，但界面选择、请求参数与后端按请求选模尚未贯通，因此暂时只能
通过修改 `.env` 的 `LLM_MODEL` 并重启后端进行全局切换。该缺口已正式纳入计划日 4。用户
随后确认第三周还需补齐会话保存、多轮上下文、SSE 和快速对话真实接入：计划日 4 负责模型
切换与会话数据基础，计划日 5 负责上下文感知 RAG、流式输出、快速对话和整周验收。跨会话
学习画像等长期记忆不属于第三周范围。
