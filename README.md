...
应用侧边栏提供上述三个第一阶段入口，并分别预留临时对话历史和课程学习助手历史；设置位于侧边栏底部。计划日 4 只实现对话入口和上下文边界说明，不伪造尚未接入模型的对话记录。
...
- 实现基础检索、重排和引用；
- 计划日 4 优先收口真实生成模型切换：从后端白名单动态读取模型、允许单次问答选择、
  服务端校验并记录实际使用模型；
- 加入查询改写、拒答和 SSE 流式输出；
- 建立第一批人工评测数据。
...
通过修改 `.env` 的 `LLM_MODEL` 并重启后端进行全局切换。该缺口已正式纳入计划日 4；查询
改写、SSE 和长期对话仍未开始。
