...
- 产品/科研源码发行分离：默认产品模式，无 Benchmark 运行依赖；科研工具与依赖显式启用。
- 智能体独立配置、不可变版本、管理 API 与界面、会话固定 revision 和诊断。
- 20260907_05 增量迁移、并发/权限/供应商/中断回归、浏览器闭环和解析开销测量。

### Fixed

- 组卷联网总开关及绑定试卷续写的范围校验。
- 前端历史恢复和流式回调的过期响应覆盖，编辑冲突保留草稿。

### Status

- Day 4/5 本轮验收已于 2026-09-10 通过，新增保留历史的智能体删除及 20260910_06 迁移；2026-09-11 用户确认本周基础任务验收完成。应用元数据已统一为 1.3.0，日志与双版 README 已获最终发布批准。
- 升级、限制和候选范围见 [v1.3.0 发布说明](docs/releases/v1.3.0.md)。
